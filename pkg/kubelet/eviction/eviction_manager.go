...
const (
	...
	//urgentEvictionGracePeriodSeconds is how long we give pods to shut down when we
	//need them to evict quickly due to resource pressure
	urgentEvictionGracePeriodSeconds = 60
)
...
type managerImpl struct {
	...
	// set of pods (format: namespace/pod: bool) that we have already tried to evict
...
func (m *managerImpl) synchronize(diskInfoProvider DiskInfoProvider, podFunc ActivePodsFunc) []*v1.Pod {
	...
	for i := range activePods {
		...
		gracePeriodOverride := int64(urgentEvictionGracePeriodSeconds)
	...
	for i := range pod.Spec.Volumes {
		...
		if source.EmptyDir != nil {
			...
			if used != nil && size != nil && size.Sign() == 1 && used.Cmp(*size) > 0 {
				...
				if m.evictPod(pod, urgentEvictionGracePeriodSeconds, fmt.Sprintf(emptyDirMessageFmt, pod.Spec.Volumes[i].Name, size.String()), nil, nil) {
...
func (m *managerImpl) podEphemeralStorageLimitEviction(podStats statsapi.PodStats, pod *v1.Pod) bool {
	...
	if podEphemeralStorageTotalUsage.Cmp(podEphemeralStorageLimit) > 0 {
		...
		if m.evictPod(pod, urgentEvictionGracePeriodSeconds, message, nil, nil) {
	...
	for _, containerStat := range podStats.Containers {
		...
		if ephemeralStorageThreshold, ok := thresholdsMap[containerStat.Name]; ok {
			if ephemeralStorageThreshold.Cmp(*containerUsed) < 0 {
				if m.evictPod(pod, urgentEvictionGracePeriodSeconds, fmt.Sprintf(containerEphemeralStorageMessageFmt, containerStat.Name, ephemeralStorageThreshold.String()), nil, nil) {
